...
def main(
    ...
    for batch_index, batch in enumerate(batches):
        ...
        with open(batch_path, "w") as batch_file:
            # NB: If we've already initialized the stories, we can just save the slug- so init_stories should happen here probably
            header = batch[0].keys()
    ...
    with open(batch_map_path, "w") as map_file:
        ...
        for domain, index in batch_map.items():
            writer.writerow({"domain": domain, "index": index})
